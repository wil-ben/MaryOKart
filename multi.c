...
/******************************************************************************/
/*  Module de Communication   client / serveur                                */
/*  En tant que serveur le programme ouvre une socket sur un port reseau      */
/*  En tant que client, le programme contact le serveur via une IP et in port */
/******************************************************************************/

// Creation de la socket. Meme procedure pour le client et le serveur
git branchint SocketCreate(void)
{
  int hSocket;
...
// Le serveur "Bind" la socket à un PORT reseau
...
// Le client se connect à la socket serveur via l'IP et le Port du serveur
int SocketConnect(int hSocket)
{
    ...
    remote.sin_addr.s_addr = inet_addr("192.168.0.36"); //Local Host
    remote.sin_family = AF_INET;
...
// Envoie des donnees via la socket (timeout of 20 seconds)
...
//reception des donnees via la socket
...
int init_communication  (int type_server, struct sockaddr_in struct_desc, int *socket_desc, int *socket)
{
  
  int clientLen ;
  //Creation de la stocket de communication. Methode identique pour le client et le serveur
  *socket_desc = SocketCreate();
  ...
  printf("Socket de communication creee\n");
  
  // test si c'est lance en tant que Client
  if(type_server == 0)
    {
      printf("Connexion en tant que client\n") ;
      //Connexion au serveur
      if (SocketConnect(*socket_desc) < 0){
        perror("connexion à la socket echouee.\n");
        return 1;
      }
  ...
  // Si c'est un serveur
  else if(type_server == 1){
    
    printf("Connexion en tant que serveur\n") ;
    //Bind
    if( BindCreatedSocket(*socket_desc) < 0)
      {
	//print the error message
	perror("bind a la socket echoue\n.");
	return 1;
      }
    printf("bind à la socket reussie\n");
    ...
    *socket=accept(*socket_desc,(struct sockaddr *)&struct_desc,(socklen_t*)&clientLen);
...
int Envoie_Message (int socket_desc, donnees_t donnees,var_t var)
{
  char message[500];
  
  // Concatenation des donnees des karts dans un message pour pouvoir l'envoyer via le reseau
  //sprintf(message,"%lf,%lf,%lf,%lf,%d,%d",donnees.modele.kart1.k.p.x,donnees.modele.kart1.k.p.y,donnees.modele.kart2.k.p.x,donnees.modele.kart2.k.p.y,c1,c2);
  sprintf(message,"%+0*lf,%+0*lf,%+0*lf,%+0*lf,%d,%d,%d,%d",11,donnees.modele.kart1.k.p.x,11,donnees.modele.kart1.k.p.y,11,donnees.modele.kart2.k.p.x,11,donnees.modele.kart2.k.p.y,var.c1,var.c2,var.a,var.b);
  
  printf("Send the Message: %s\n", message);
  
  //envoie du message a l'autre joueur
  SocketSend(socket_desc, message, strlen(message));
  
  return 0 ;
}

int Lire_Message (int sock, struct sockaddr_in client, donnees_t *donnees,var_t *var)
{
  char message [500]="" ; // contient le double convertit en char
  char *endptr="";
  char c_kart1_x [11] ="" ;
  char c_kart1_y [11] ="" ;
  char c_kart2_x [11] ="" ;
  ...
  // Extraire de la chaine de caratere chaque objet kart1, kart2, cube1, cube2, position x et position y
  strncpy (c_kart1_x, message,10) ;
  strncpy (c_kart1_y, &message [12],10) ;
  strncpy (c_kart2_x, &message [24],10) ;
  strncpy (c_kart2_y, &message [36],10) ;
  strncpy (c_c1, &message [48],1) ;
  strncpy (c_c2, &message [50],1) ;
  strncpy (c_a, &message [52],1) ;
  strncpy (c_b, &message [54],1) ;
  /* En cas de Debug */
  printf ("%s\n", c_kart1_x) ;
  printf ("%s\n", c_kart1_y) ;
  printf ("%s\n", c_kart2_x) ;
  ...
  donnees->modele.kart1.k.p.x=strtod (c_kart1_x, &endptr) ;
  donnees->modele.kart1.k.p.y=strtod (c_kart1_y, &endptr) ;
  donnees->modele.kart2.k.p.x=strtod (c_kart2_x, &endptr) ;
  donnees->modele.kart2.k.p.y=strtod (c_kart2_y, &endptr) ;
...
int main(int argc, char**argv){
  ...
  int socket_desc ;
  int socket ;
  int type_server=0 ; // si c'est un client type_server=0, si c'est un serveur type_server=1
  ...
  if (strcmp (argv[1], "-c") == 0)
    ...
    {
      printf("il faut choisir -c pour client et -s pour serveur\n") ;
    }
  init_communication (type_server, struct_socket, &socket_desc, &socket);
  ...
  charger_im_kart(&donnees,&var);//charge les images des karts selectionnés
  /* boucle d'animation */
  while (!fini(donnees)){
    ...
    /* créer l'image sur le buffer */
     if (type_server == 1){
      // Lecture des informations des donnees de l'autre Kart
      Lire_Message (socket, struct_socket, &donnees, &var) ;
      printf("Message recu (Main : apress Lire_Message) %lf,%lf,%lf,%lf,%d,%d,%d,%d\n",donnees.modele.kart1.k.p.x,donnees.modele.kart1.k.p.y,donnees.modele.kart2.k.p.x,donnees.modele.kart2.k.p.y,var.c1,var.c2,var.a,var.b);
    }
    else{
      //envoie de message
      Envoie_Message (socket_desc,donnees,var) ;
    }
...
modele_t init_modele(var_t var){
  donnees_t donnees;
  int o=0;
  //different fichier sont lus en focntion de la map choisie
  if(var.map==2){
...
void mettre_a_jour(donnees_t *donnees,etat_t *etat,var_t *var){
  ...
  etat->f = fopen("maj.c","r");//contient la vitesse et le temps durant lequel les objet sont actif
  while(!feof(etat->f)){
    fscanf(etat->f,"%d %lf %lf %lf\n",&etat->t_pi[o],&etat->c_pi1[o],&etat->c_pi2[o],&etat->c_pi3[o]);
    o++;
  }
  fclose(etat->f);
  //les fichiers lu sont la couleur des pixels autour de la route pour le respawn
  ...
  if(var->d1==1||var->f1==1 || var->s1==1){
	...
	var->v=1;
	donnees->son.p[7]=jouerSon(donnees->son.s_tab[7],0);/*fait disparaitre*/
        volumePiste(donnees->son.p[7],1);                   /* le kart 2      */  
      }
    }
  }
...
void afficher_menu(donnees_t *donnees,var_t *var){
  ...
  afficherImage(donnees->image.im_tab[5],0-tn_largeur(donnees->image.im_tab[5])/2,0+tn_hauteur(donnees->image.im_tab[5])/2);//affiche l'image d'acceuil
  tamponner();
  donnees->son.p[0]=jouerSon(donnees->son.s_tab[0],5);//musique du menu
  donnees->son.p[9]=jouerSon(donnees->son.s_tab[8],0);// voix "Welcome to mario kart"
  ...
  while(u==0){
    ...
    if(esc.touchesClavier[40]==1){
      u++;
      effacerTableau();
      afficherImage(donnees->image.im_tab[6],0-tn_largeur(donnees->image.im_tab[6])/2,0+tn_hauteur(donnees->image.im_tab[6])/2);//affiche l'image de selection des personnages
      donnees->son.p[8]=jouerSon(donnees->son.s_tab[9],0);//voix"select your player"
      volumePiste(donnees->son.p[8],1);
      tamponner();
      while(o!=2){
	esc = lireEtatSourisClavier();
	if(esc.touchesClavier[30+i]==1){//touche pour sélectionner les personnage
	  if(o==0){//le joueur 1 choisit son personnage
	    if(i==9){//touche 0
	      var->perso1=0;
	    }else if(i==-10){//touche A
	      var->perso1=40;
	    }else if(i==-4){//touche Z
	      var->perso1=44;
	    }else if(i==-22){//touche E
	      var->perso1=48;
	    }else if(i==-9){//touche R
	      var->perso1=52;
	    }else if(i==-7){//touche T
	      var->perso1=56;
	    }else{
	      var->perso1=(i+1)*4;
	    }
	    charger_son_kart(donnees,var);//voix du personnage choisi
	    effacerTableau();
	    ...
	    o++;
	    attendreNms(300);
	  }else{//le joueur 2 chosit son personnage (meme touche que pour le joueur 1)
	    if(i==9){
	      var->perso2=0;
	    }else if(i==-10){
	      var->perso2=40;
	...
	i=0;
	effacerTableau();
	afficherImage(donnees->image.im_tab[0],0-tn_largeur(donnees->image.im_tab[0])/2,0+tn_hauteur(donnees->image.im_tab[0])/2);//affiche l'image de la selection des map
	jouerSon(donnees->son.s_tab[10],0);// voix "select the map"
	tamponner();
	attendreTouche();
	while(o==0){
	  esc = lireEtatSourisClavier();
	  if(esc.touchesClavier[30+i] ==1){//touche pour choisir les maps (1 à 9)
	    var->map=i;
	    o++;
	  }
	  if(i==8){
    ...
    }else if(esc.touchesClavier[224]==1){//touche controle
      effacerTableau();
      u++;
      afficherImage(donnees->image.im_tab[8],0-tn_largeur(donnees->image.im_tab[8])/2,0+tn_hauteur(donnees->image.im_tab[8])/2);//affiche les commandes
      tamponner();
      while(o!=1){
	esc = lireEtatSourisClavier();
...
void afficher_score_fin(donnees_t *donnees,var_t *var){
  ...
  if((var->a == 4 || var->b==8)){
    while(i==0){
      if(u==0){
	donnees->son.p[10]=jouerSon(donnees->son.s_tab[11],0);//voix "congratulation"
	volumePiste(donnees->son.p[10],1);
	donnees->son.p[1]=jouerSon(donnees->son.s_tab[6],5);//musique de victoire
	volumePiste(donnees->son.p[1],1);
	u++;
	supprimerPiste(donnees->son.p[2]);
	if(var->a==4){//si le joueur 1 gagne
	  donnees->image.im_k[var->pos1+o]=rotozoomImage(donnees->image.im_k[var->pos1+o],0,3,3);//affiche le kart 1 en gros
	  ...
	  donnees->image.im_k[var->pos2+j]=rotozoomImage(donnees->image.im_k[var->pos2+j],0,3,3);//affiche le kart 2 en grod
	}
      }
      afficherImage(donnees->image.image_tab[4],0-tn_largeur(donnees->image.image_tab[4])/2,0+tn_hauteur(donnees->image.image_tab[4])/2);//affiche l'image de fin de jeu
      if(y==0){
	donnees->t=0;
	y++;
      }
	...
	*var=init_var(*donnees);
	afficher_menu(donnees,var);
	charger_im_map(donnees,var);
	*donnees=init_donnees(donnees,*var);//relance le jeu
  ...
  if(var->a<=3){
	...
	fixerCouleurDeTransparence(donnees->image.image_tab[9],0,0,0);
	}*/
